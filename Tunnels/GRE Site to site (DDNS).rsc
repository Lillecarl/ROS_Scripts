...
:local OldLocalAddr [/interface gre get [find comment=$CommentIDF ] local-address]
:local OldRemoteAddr [/interface gre get [find comment=$CommentIDF ] remote-address]

if ($NewLocalAddr != $OldLocalAddr) do={
    /interface gre set [find comment=$CommentIDF] remote-address=$NewLocalAddr
    /log info "Local tunnel IP for $CommentIDF has changed from $OldLocalAddr to $NewLocalAddr"
}

if ($NewRemoteAddr != $OldRemoteAddr) do={
    /interface gre set [find comment=$CommentIDF] remote-address=$NewRemoteAddr
